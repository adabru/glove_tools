...
while len(SORTED_IMUS) < len(PARENT_REL):
    SORTED_IMUS += [
        key
        for key, (parent, offset) in PARENT_REL.items()
...
def get_model_path(model_name) -> Path:
    return Path(__file__).parent / f"models/{model_name}.egg"


def get_recording_path() -> Path:
    return Path(__file__).parent / "recording.pickle"
