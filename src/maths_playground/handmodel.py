from panda3d.core import LColor, PandaNode, Quat, Vec3
...
class Handmodel:
    ...
    def __init__(self, showBase, name, isJoint):
        ...
        modelBone = showBase.loader.loadModel(get_model_path("bone"))
        for key in IMU_POS:
    ...
    def update(self, data, quatId):
        for name in data:
            ...
            color = {
                ImuState.Booting: LColor(0.2, 0.2, 0, 0.2),
                ImuState.NotConnected: LColor(0.2, 0.2, 0.2, 0.2),
                ImuState.Error: LColor(1, 0, 0, 1),
                ImuState.Running: None,
                ImuState._Noexist: LColor(0.04, 0.04, 0.04, 0.04),
            ...
            if not self.isJoint:
                ...
                if parent == None:
                    ...
                    self.bones[name].setPos(
                        self.bones[parent].getPos() + Vec3(0, 4, 0) + offset
                    )
                    self.bones[name].setQuat(data[name][quatId])
                else:
                    self.bones[name].setPos(
                        self.bones[parent].getPos()
                        + self.bones[parent].getQuat().xform(Vec3(0, 4, 0) + offset)
                    )
                    quat = data[name][quatId] * self.bones[parent].getQuat()
